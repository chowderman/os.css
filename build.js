...
function buildJSLibraries() {
  let combinedJSCode = "";

  globby.sync("js/*.js").forEach(filePath => {
    let jsCode = fs.readFileSync(filePath).toString();
    let uglifyResult = UglifyJS.minify(jsCode);
    if (!uglifyResult.error) {
      combinedJSCode += uglifyResult.code;
    }
  });

  fs.writeFileSync("dist/os.js", combinedJSCode);
}
